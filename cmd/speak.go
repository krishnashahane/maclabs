...
func init() {
	...
	cmd := &cobra.Command{
		...
		Long:  "If no text argument is provided, the command reads from stdin.\n\nTip: run `maclabs prompting` for model-specific prompting tips and recommended flag combinations.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if voiceInput == "" {
				if env := os.Getenv("ELEVENLABS_VOICE_ID"); env != "" {
					voiceInput = env
					forceVoiceID = true
				} else if env := os.Getenv("MACLABS_VOICE_ID"); env != "" {
					voiceInput = env
					forceVoiceID = true
				}
			}
...
func resolveVoice(ctx context.Context, client *elevenlabs.Client, voiceInput string, forceID bool) (string, error) {
	...
	return "", fmt.Errorf("voice %q not found; try 'maclabs voices' or -v '?'", voiceInput)
}
