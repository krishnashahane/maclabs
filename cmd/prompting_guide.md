...
- Some English-only models support SSML `<phoneme>` for pronunciation (not yet exposed in `maclabs`).
...
```
maclabs speak -v Roger --stability 0.5 --similarity 0.75 --style 0.0 --normalize auto --lang en \
  "We shipped today. It was close… but it worked."
```

Fast + cheap (v2.5 Flash):
```
maclabs speak -v Roger --model-id eleven_flash_v2_5 --stability 0.5 --normalize auto --lang en \
  "Short. Crisp. Low latency."
```

Expressive (v3):
```
maclabs speak -v Roger --model-id eleven_v3 --stability 0.35 --normalize off --lang en \
